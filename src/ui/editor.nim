...
proc handleInput*(ed: var Editor, pressed: set[ButtonInput], instant: set[InstantInput]): bool =
  ...
  if E_Left in pressed and E_Right in pressed:
    hideEditorSprites()
    return true
  if not (E_Y in pressed and E_X in pressed):
    if ed.currentUI == pickerUI:
      closePicker()
  ...
  if E_X in pressed:
    if E_Y in pressed:
      ...
      if E_Left in newPressed or E_ScrollUp in instant:
        ...
        ed.updatePalette()
        ed.deferSave()
  case ed.currentUI: #TODO idk WHAT's going on here
    of editorUI:
      if E_Up in newPressed and not(E_X in pressed):
        ed.moveCursor(0, +1)
      elif E_Down in newPressed and not(E_X in pressed):
        ed.moveCursor(0, -1)
      elif E_B in pressed:
        ...
        ed.moveCursor(+1, 0)
      elif E_A in pressed and not (E_X in pressed):
